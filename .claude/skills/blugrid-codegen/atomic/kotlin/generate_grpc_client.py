...
"""
gRPC Client Generator Skill

Generates gRPC client wrapper class.

Usage:
    python generate_grpc_client.py --schema schema.yaml --output OrganisationGrpcClient.kt
"""
...
def generate_grpc_client(schema: dict, output_path: str) -> str:
    """
    Generate a Kotlin gRPC client wrapper file.
    ...
    """
    template_path = Path(__file__).parent.parent.parent / "templates" / "kotlin" / "grpc_client.kt.j2"
...
def main():
    parser = argparse.ArgumentParser(description="Generate Kotlin gRPC Client")
    ...
    generate_grpc_client(schema, args.output)
